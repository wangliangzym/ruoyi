  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="性别" prop="teacherSex">
        <el-select v-model="queryParams.teacherSex" placeholder="请选择性别" clearable size="small">
          <el-option
            v-for="dict in teacherSexOptions"
            :key="dict.dictValue"
    ...
    <el-table v-loading="loading" :data="teacherList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="性别" align="center" prop="teacherSex" :formatter="teacherSexFormat" />
      <el-table-column label="图像" align="center" prop="photo" />
...
export default {
  ...
  created() {
    ...
    this.getDicts("sys_user_sex").then(response => {
      this.teacherSexOptions = response.data;
    });
  },
  methods: {
    /** 查询教师信息列表 */
    ...
    // 性别字典翻译
    teacherSexFormat(row, column) {
      return this.selectDictLabel(this.teacherSexOptions, row.teacherSex);
    },
    // 取消按钮
    cancel() {
      this.open = false;
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除教师信息编号为"' + ids + '"的数据项?', "警告", {
